...
- (void)generate
{
    ...
    [PLDelgateHelper bindTarget:_streamingSession property:@"delegate" block:^(A2DynamicDelegate *d) {
        ...
        [d implementMethod:@selector(mediaStreamingSession:microphoneSourceDidGetAudioBuffer:) withBlock:^AudioBuffer *(PLMediaStreamingSession *session, AudioBuffer* audioBuffer){
            __strong typeof(wSelf) strongSelf = wSelf;
            if ([strongSelf.delegate respondsToSelector:@selector(panelDelegateGenerator:microphoneSourceDidGetAudioBuffer:)]) {
                audioBuffer = [strongSelf.delegate panelDelegateGenerator:strongSelf microphoneSourceDidGetAudioBuffer:audioBuffer];
            }
            return audioBuffer;
        }];
...
//                NSLog(@"%@", [NSString stringWithFormat:@"session status %@", status]);
        ...
        [d implementMethod:@selector(mediaStreamingSession:cameraSourceDidGetPixelBuffer:timingInfo:) withBlock:^CVPixelBufferRef(PLMediaStreamingSession *session,CVPixelBufferRef pixelBuffer,CMSampleTimingInfo timeInfo){
            __strong typeof(wSelf) strongSelf = wSelf;
            if ([strongSelf.delegate respondsToSelector:@selector(panelDelegateGenerator:cameraSourceDidGetPixelBuffer:timingInfo:)]) {
                pixelBuffer = [strongSelf.delegate panelDelegateGenerator:strongSelf cameraSourceDidGetPixelBuffer:pixelBuffer timingInfo:timeInfo];
            }
            return pixelBuffer;
        }];
    }];
