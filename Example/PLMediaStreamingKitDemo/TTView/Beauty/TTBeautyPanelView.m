...
- (void)facePlasticPanelView:(TTFacePlasticPanelView *)view didSelectItem:(TTFacePlasticItem *)item
{
    ...
    if (item.isReset) {
        ...
        UIAlertAction *cancelAction = [UIAlertAction actionWithTitle:NSLocalizedStringFromTable(@"tu_取消", @"VideoDemo", @"取消") style:UIAlertActionStyleCancel handler:^(UIAlertAction * _Nonnull action) {
            
            self->_paramtersAdjustView.hidden = YES;
        }];
        
        UIAlertAction *sureAction = [UIAlertAction actionWithTitle:NSLocalizedStringFromTable(@"tu_确定", @"VideoDemo", @"确定") style:UIAlertActionStyleDefault handler:^(UIAlertAction * _Nonnull action) {
